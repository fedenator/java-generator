...
module.exports =
class Parser
    varRegexArray: /(public|private|protected)\s*(static)?\s*(final)?\s*(volatile|transient)?\s*([a-zA-Z0-9_$]+)(\<.*?\>)?\s*([a-zA-Z0-9_$,\s]+)/g
    varRegex: /(public|private|protected)\s*(static)?\s*(final)?\s*(volatile|transient)?\s*([a-zA-Z0-9_$]+)(\<.*?\>)?\s*([a-zA-Z0-9_$,\s]+)/
